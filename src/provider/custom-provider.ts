...
const ChainInputSchema = v.object({
  ...
  fees: v.object({
    fee_tokens: v.array(v.object({ denom: v.string() })),
  }),
  metadata: v.optional(
    v.object({
...
function normalizeAssetInput(raw: Record<string, unknown>, chainId: string): AssetInfo {
  ...
  const denom = String(raw.denom ?? raw.base ?? '')
...
export class CustomProvider extends BaseChainDataProvider {
   ...
   * @throws {ValiError} If input fails schema validation
   */
  static from(input: string | ChainInput | ChainInput[]): CustomProvider {
    ...
    for (const item of items) {
      const validated = v.parse(ChainInputSchema, item)
  ...
  addAssetList(input: AssetListInput | string): void {
    const data = typeof input === 'string' ? (JSON.parse(input) as AssetListInput) : input
    const chainId = String(data.chainId ?? data.chain_id ?? '')
    if (!chainId) throw new Error('chainId or chain_id is required in asset list input')
  ...
  get bridge(): Bridge {
    if (!this._bridge) {
      const firstChain = this.chains.values().next().value
