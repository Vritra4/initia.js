 ...
 *
 * Used by entry points (entry.browser.ts, entry.node.ts) and from-chain-standalone.ts
...
function resolveConfig(chainType: string, network?: string) {
  if (chainType === 'other') {
  ...
  return network ? config.build(network) : config.build()
}
...
function getCached(chainInfo: ChainInfo) {
  ...
  if (!cached) {
    cached = resolveConfig(chainInfo.chainType, chainInfo.network)
