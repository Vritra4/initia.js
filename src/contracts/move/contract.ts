...
/**
 * Converts a single arg to a JSON string, auto-converting bech32 addresses to hex
 * for address-typed parameters.
 */
...
export function createScriptMsg(options: ExecuteScriptOptions): MsgScriptJSON {
  const jsonArgs = (options.args ?? []).map(arg => {
    // If already Uint8Array, convert to hex string for JSON
...
export async function buildMoveView(
  ...
  if (options.paramTypes) {
    ...
    jsonArgs = args.map(arg => JSON.stringify(arg))
  }
