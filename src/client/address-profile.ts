...
export type AddressProfile =
  ...
  | {
      ...
      account: 'evm-code'
      contract: 'evm'
      /** Hex string length of deployed EVM code (not byte count). 0 means no code. */
      codeSize: number
      codeHash: Uint8Array
    }
  | {
 ...
 * Note: returns hex string length (not byte count). Consistent with deprecated evmCodeSize.
 */
