 ...
 * For raw client-only access without ChainContext, see {@link fromChain}.
 *
...
import { makeStdSignDoc, makeAminoSignBytes, type StdFee } from '../tx/sign'
 ...
 */
interface BaseChainContext<T extends ChainType = ChainType> {
  /** Chain identifier */
   ...
   */
  withSigner(signer: DirectSigner | AminoSigner): ChainContext<T>
   ...
   */
  forAddress(address: string): ChainContext<T>
 ...
 * const ctx: ChainContext = wallet.chain('some-chain')
 *
 * // Narrowed — evmRpc available
 * const ctx: ChainContext<'minievm'> = wallet.chain<'minievm'>('evm-1')
 * ctx.evmRpc.getBalance('0x...')   // OK, no '?' needed
 * ctx.client.evm.call(...)         // OK, evm service autocompletes
 * ```
 */
export type ChainContext<T extends ChainType = ChainType> = BaseChainContext<T> &
  ChainContextExtrasMap[T]

// ============================================================================
// Transaction Encoding Helpers (used by sign())
// ============================================================================

/**
 * Encode an UnsignedTx into bodyBytes + authInfoBytes for direct signing.
 */
function encodeTxDirect(
  tx: UnsignedTx,
  pubKey: Uint8Array,
  ...
  const fee = create(FeeSchema, {
    amount: tx.fee.map(coin => ({ denom: coin.denom, amount: coin.amount })),
...
/**
 * Build StdFee from UnsignedTx fields.
 */
function buildStdFee(tx: UnsignedTx): StdFee {
  return {
    amount: tx.fee,
...
class ChainContextImpl<T extends ChainType> implements BaseChainContext<T> {
  ...
  constructor(
    ...
    // Address: from Key (sync), explicitly provided, or lazy resolved from signer
    if (this._signer instanceof Key) {
      const prefix = chainInfo.bech32Prefix ?? 'init'
      this._address = bech32.encode(prefix, bech32.toWords(this._signer.rawAddress))
    } else {
  ...
  private async getSignerAddress(signerOverride?: DirectSigner | AminoSigner): Promise<string> {
    ...
    // Key: sync address derivation (no await needed)
    if (signer instanceof Key) {
      const prefix = this.chainInfo.bech32Prefix ?? 'init'
      return bech32.encode(prefix, bech32.toWords(signer.rawAddress))
    }

    // External signer: lazy cache
    if (!this._address) {
      const prefix = this.chainInfo.bech32Prefix ?? 'init'
  ...
  private requireAddress(addressOverride?: string): string {
    const addr = addressOverride ?? this._address
    if (!addr) {
      throw new Error(
  ...
  withSigner(signer: DirectSigner | AminoSigner): ChainContext<T> {
    return new ChainContextImpl<T>(this.chainInfo, this.client, this.msgs, {
      signer,
      ...this.carryover(),
    }) as ChainContext<T>
  }
  ...
  async getAccount(options?: GetAccountOptions): Promise<AccountInfo> {
    const addr = this.requireAddress(options?.address)
    return getAccount(
      this.client,
      addr,
  ...
  async createTx(msgs: MsgInput[], options?: SignOptions): Promise<UnsignedTx> {
    const address = await this.getSignerAddress(options?.signer)
   ...
   * AminoSigner-only → 'amino', otherwise → 'direct'
   */
  ...
  private async signWithAmino(
    ...
    const aminoSignDoc: AminoSignDoc = {
      ...
      fee: { amount: tx.fee, gas: tx.gasLimit.toString() },
      msgs: aminoMsgs,
      memo: tx.memo,
    }
  ...
  async getTokenInfo(options: GetTokenInfoOptions): Promise<TokenInfo> {
    ...
    // TODO: pass options.height when TokenContract supports it
