...
describe.skipIf(SKIP)('Bridge (Integration)', () => {
  ...
  // ==========================================================================
  // Wallet.bridge accessor (real provider, query-only)
  // ==========================================================================

  describe('Wallet.bridge', () => {
    it('should fetch withdrawals via wallet.bridge (auto address derive)', async () => {
      const l2WithExecutor = l2Chains.find(c => c.executorUri)!

      const key = new MnemonicKey({ mnemonic: TEST_MNEMONIC })
      const wallet = new Wallet(createChainContext, { key, provider })

      const withdrawals = await wallet.bridge.getWithdrawals(l2WithExecutor.chainId)
      expect(Array.isArray(withdrawals)).toBe(true)
    }, 60000)
  })
  ...
  describe.skipIf(!TX_MNEMONIC)('deposit → withdraw round-trip', () => {
    ...
    beforeAll(() => {
      const key = new MnemonicKey({ mnemonic: TX_MNEMONIC! })
    ...
    it('should deposit L1 → L2 and confirm tx', async () => {
      const result = await wallet.bridge.deposit(targetL2.chainId, '1000uinit')
    ...
    it('should withdraw L2 → L1 and confirm tx', async () => {
      // Use L2's native denom (queried from chain or a small amount)
      const result = await wallet.bridge.withdraw(targetL2.chainId, '1000uinit')
    ...
    it('should have withdrawal visible via getWithdrawals after withdraw tx', async () => {
      // After the withdraw tx above, the withdrawal should appear in the executor API
      // Note: there may be a short delay before the executor indexes the tx
      const withdrawals = await wallet.bridge.getWithdrawals(targetL2.chainId)

      expect(Array.isArray(withdrawals)).toBe(true)
      // At minimum, the withdrawal we just created should exist
      // (may include prior withdrawals too)
