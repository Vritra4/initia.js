...
describe('createVip', () => {
  ...
  describe('execute methods', () => {
    ...
    it('undelegate: amount undefined should default to 0n', () => {
      const vip = createVip(makeMockCtx())
      vip.undelegate({
        metadata: '0x1::native_uinit::Coin',
        releaseTime: 1700000000,
      ...
      expect(executeCalls[0].args[1]).toBe(0)
    })

    it('provideAndDelegate: minLiquidity undefined should default to 0n', () => {
      const vip = createVip(makeMockCtx())
      ...
      expect(executeCalls[0].args[3]).toBe(0) // minLiquidity
    })
