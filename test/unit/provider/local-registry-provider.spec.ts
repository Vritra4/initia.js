...
function setupTmpRegistry() {
  ...
  // mainnets/bad-json/ (invalid JSON, should be skipped)
...
describe('LocalRegistryProvider', () => {
  ...
  describe('construction', () => {
    ...
    it('skips invalid JSON files', () => {
      // bad-json/chain.json has invalid JSON — should be silently skipped
      const chains = provider.listChains()
      const badChain = chains.find(c => c.chainId === 'bad-json')
      expect(badChain).toBeUndefined()
    })
  })
