...
describe('RegistryProvider', () => {
  ...
  describe('fetch failure handling', () => {
    it('should return empty chains when chains.json fetch throws network error', async () => {
      vi.restoreAllMocks()
      vi.stubGlobal(
        'fetch',
        vi.fn(async () => {
      ...
      const p = await createRegistryProvider({ network: 'mainnet', timeout: 1000 })
      expect(p.listChains()).toHaveLength(0)
    })

    it('should return empty chains when chains.json returns 500', async () => {
      vi.restoreAllMocks()
      vi.stubGlobal(
        'fetch',
        vi.fn(async () => {
