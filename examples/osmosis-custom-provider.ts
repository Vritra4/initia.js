...
import { fromChain, type BaseClient } from 'initia.js/client'
import { CustomProvider, type ChainInfo } from 'initia.js/provider'
...
// =============================================================================
// Helper: Create client from ChainInfo
// =============================================================================

/**
 * Create a gRPC client from ChainInfo.
 * Uses fromChain with appropriate provider.
 */
function createClientFromChainInfo(chainInfo: ChainInfo): BaseClient {
  const result = fromChain(chainInfo.chainId, {
    provider: new CosmosRegistryProvider({ chainNames: [chainInfo.chainName ?? 'unknown'] }),
  })
...
async function queryOsmosis(chainInfo: ChainInfo) {
  console.log('=== Example 4: Query Osmosis ===\n')

  // Create gRPC client (uses BaseClient for 'other' chainType)
  const client = createClientFromChainInfo(chainInfo)
...
async function queryOsmosisSpecific(_chainInfo: ChainInfo) {
  ...
  console.log()
  console.log('2. Import and use Osmosis services:')
  console.log(`
   import { createGrpcClient } from 'initia.js/client'
   import { Query as PoolManagerQuery } from '@buf/osmosis-labs_osmosis.connectrpc_es/...'

   const grpcClient = createGrpcClient(chainInfo)
   const poolManager = grpcClient.service(PoolManagerQuery)
   const pools = await poolManager.pools({})
  `)
  console.log()
